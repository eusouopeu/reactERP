...
const calculateTotalGasto = (
	transactionsData: Array<{ valorNotaFiscal: string }>,
): number => {
	return transactionsData.reduce((total, item) => {
		const valor = parseFloat(
			item.valorNotaFiscal.replace('.', '').replace(',', '.'),
		)
		return total + valor
	}, 0)
}
...
const fornecedorMaiorGasto = (
	transactionsData: Array<{
		nomeFornecedor: string
		valorNotaFiscal: string
	}>,
): { nomeFornecedor: string; valor: number } | null => {
	const fornecedor = transactionsData.reduce(
		(max, item) => {
			const valor = parseFloat(
				item.valorNotaFiscal.replace('.', '').replace(',', '.'),
			)
...
const municipioComMaisTransacoes = (
	transactionsData: Array<{ municipioFornecedor: string }>,
): { municipio: string; quantidade: number } | null => {
	...
	const municipioMaisTransacoes = Object.entries(
		transacoesPorMunicipio,
	).reduce(
		(max, [municipio, quantidade]) => {
	...
	return municipioMaisTransacoes.quantidade > 0
		? municipioMaisTransacoes
		: null
}
