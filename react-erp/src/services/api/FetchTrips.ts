    ...
    const promise = axios.get(url, { params, headers })
      .then((response: AxiosResponse<any[]>) => {
        ...
        const trips = response.data.map((trip: any) => ({
          ...
          cargo: trip.cargo.descricao.toUpperCase().includes('PROFESSOR')
            ? 'Professor'
            : trip.cargo.descricao.toUpperCase().includes('ASSISTENTE') ? 'Tec. Admin.' : trip.cargo.descricao.toLowerCase(),
          unidade: trip.unidadeGestoraResponsavel.nome,
          dataInicio: Intl.DateTimeFormat('pt-BR').format(new Date(trip.dataInicioAfastamento)),
          dataFim: Intl.DateTimeFormat('pt-BR').format(new Date(trip.dataFimAfastamento)),
          valorDiarias: `R$ ${trip.valorTotalDiarias}`,
          valorPassagens: `R$ ${trip.valorTotalPassagens}`,
          valorTotal: `R$ ${Intl.NumberFormat('pt-BR').format(trip.valorTotalViagem)}`,
        }));

        allTrips = [...allTrips, ...trips];
      })
      .catch(error => {
        console.error(`Erro ao buscar dados de viagens para o intervalo ${range.dataIdaDe} - ${range.dataRetornoAte}:`, error);
        throw error;
      });
