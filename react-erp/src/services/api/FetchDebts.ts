...
export const fetchDebts = (): Promise<DebtModel[]> => {
  const url = 'https://api.portaldatransparencia.gov.br/api-de-dados/despesas/por-orgao';
  const headers = {
    'Accept': '*/*',
    ...
    const promise = axios.get(url, { params, headers })
      .then((response: AxiosResponse<any[]>) => {
        ...
        const debt: DebtModel[] = response.data.map((item: any) => ({
          ano: item.ano,
          empenhado: parseFloat(item.empenhado.replaceAll('.', '').replace(',', '.')),
          liquidado: parseFloat(item.liquidado.replaceAll('.', '').replace(',', '.')),
          pago: parseFloat(item.pago.replaceAll('.', '').replace(',', '.')),
        }));
