...
export default function StaffTable() {
  ...
  const onChange: TableProps<StaffModel>['onChange'] = (pagination, filters, sorter, extra) => {
    console.log('params', pagination, filters, sorter, extra)
  }

  const getColumnSearchProps = (dataIndex: DataIndex): TableColumnType<StaffModel> => ({
  ...
  const receiptsTableHeader: TableColumnsType<StaffModel> = [
    ...
    },
    {
      ...
      render: (_, { cargo }) => (
        cargo === 'Professor'
          ? <Tag className='tag-professor' key={cargo}>
            {cargo}
          </Tag>
          : cargo === 'Tec. Admin.' ?
          <Tag className='tag-ta' key={cargo}>
            {cargo}
          </Tag> : <Tag className='tag-outros' key={cargo}>
            {cargo}
          </Tag>
      ),
      filterSearch: true,
    ...
    },
    {
      ...
      // ),
      filterSearch: true,
      onFilter: (value, record) => record.unidade.includes(value as string),
    },
    {
      title: 'Admissão',
      dataIndex: 'admissao',
    ...
    },
    {
      ...
      sorter: {
        compare: (a: StaffModel, b: StaffModel) => parseFloat(a.salario.replace('R$ ', '').replace('.','').replace(',', '.')) - parseFloat(b.salario.replace('R$ ', '').replace('.','').replace(',', '.')),
      },
    },
  ]
